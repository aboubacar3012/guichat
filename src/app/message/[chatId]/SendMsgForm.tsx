...
const SendMsgForm = (
  ...
  const [disabledSendMsg, setDisabledSendMsg] = useState(false);
  const [username, setUsername] = useState('');
  const auth = useSelector((state: RootState) => state.auth);


  useEffect(() => {
    if (auth.isAuthenticated && auth?.user?.username) setUsername(auth?.user?.username);
  }, []);
  ...
  return (
    <div>
      ...
      />
      {
        disabledSendMsg
          ?
          <svg aria-hidden="true" className="w-10 h-10 absolute right-4 top-6 text-gray-200 animate-spin  fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
